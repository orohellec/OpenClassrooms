...
class Joueur < Personne
  ...
  def degats
    # A faire:
    # - Calculer les dégats
	degats = self.points_de_vie * 0.5 + self.degats_bonus
    # - Affiche ce qu'il s'est passé
	puts degats
	return degats
  end
  ...
  def soin
    ...
    # - Gagner de la vie
    # - Affiche ce qu'il s'est passé
	self.points_de_vie += 22
	puts "#{nom} regagne 22 points de vie"
  end
...
class Jeu
  ...
  def self.est_fini(joueur, monde)
	...
	i = 0
	nb_ennemis_vivants = 3
	while (i < monde.ennemis.length)
		if (monde.ennemis[i].en_vie == false)
	...
	if (joueur.en_vie == false)
		return 0
	elsif (nb_ennemis_vivants == 0)
		puts "Tous les ennemis ont été vaincus"
		return 0
	end
  end
end
...
class Monde
  ...
  def ennemis_en_vie
	...
	while (i < ennemis.length)
		if ennemis[i].en_vie == true
			copy_ennemis[j] = ennemis[i]
			i += 1
			j += 1
		else
			i += 1
...
puts "Il vous reste #{joueur.points_de_vie} hp"
monde.ennemis.each do |ennemi|
	puts "#{ennemi.nom}: #{ennemi.points_de_vie} hp"
end
